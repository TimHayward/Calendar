...
async function scrape() {
  ...
  if (cfg.debug?.enabled) {
    ...
    try { fs.writeFileSync(cfg.debug.htmlPath, await page.content(), "utf8"); } catch {}
    try { await page.screenshot({ path: cfg.debug.screenshotPath, fullPage: true }); } catch {}
    try { fs.writeFileSync(cfg.debug.networkLogPath, JSON.stringify(captured, null, 2)); } catch {}
  }
